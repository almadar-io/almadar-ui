...
export function inferDirection(dx: number, dy: number): FacingDirection {
    ...
    // Primary axis: whichever delta is larger determines the quadrant
    if (dx >= 0 && dy >= 0) return 'se'; // Moving +x, +y → toward lower-right
    if (dx <= 0 && dy >= 0) return 'sw'; // Moving -x, +y → toward lower-left
    if (dx >= 0 && dy <= 0) return 'ne'; // Moving +x, -y → toward upper-right
    return 'nw';                          // Moving -x, -y → toward upper-left
}
...
export function resolveFrame(
    sheetUrls: { se: string; sw: string } | null,
...
export function transitionAnimation(
    ...
    // Don't restart the same animation (unless it's a new one-shot)

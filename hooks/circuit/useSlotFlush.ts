...
import type { ClientEffectTuple, EventPayload, PatternNode } from '@almadar/core';
...
export interface SlotFlushHandle {
  ...
  applyClientEffects: (
    ...
    clientEffectsByTrait: ReadonlyArray<{ traitName: string; effect: ClientEffectTuple }> | undefined,
...
export function useSlotFlush(
  ...
  const applyClientEffects = useCallback((
    ...
    const slots = uiSlotsRef.current;
    const embedded = embeddedTraitsRef.current;
    const tuples: Array<{ effect: ClientEffectTuple; traitName?: string }> = clientEffectsByTrait
      ? clientEffectsByTrait.map((entry) => ({ effect: entry.effect, traitName: entry.traitName }))
    ...
    for (const { effect, traitName } of tuples) {
      ...
      if (kind === 'render-ui') {
        ...
        const isEmbedded = embedded?.has(sourceTrait) ?? false;
        ...
        if (pattern === null) {
          ...
          slots.updateTraitContent(sourceTrait, { pattern: patternType as string, props, priority: 0, animation: 'fade' });
        } else {
          slots.render({ target: slot as Parameters<typeof slots.render>[0]['target'], pattern: patternType as string, props, sourceTrait });
        }
  ...
  return { flushSlot, applyClientEffects };
}
